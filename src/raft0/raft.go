...
func (rf *Raft) CondInstallSnapshot(lastIncludedTerm int, lastIncludedIndex int, snapshot []byte) bool {
	...
	if lastIncludedIndex > rf.getLastIndex() {
		...
		rf.log = rf.log[lastIncludedIndex-rf.getFirstIndex():]
...
func (rf *Raft) Snapshot(index int, snapshot []byte) {
	...
	rf.mu.Lock()
	defer rf.mu.Unlock()
	snapshotIndex := rf.getFirstIndex()
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if rf.log[args.PrevLogIndex-firstIndex].Term != args.PrevLogTerm {
		...
		for i := args.PrevLogIndex; i >= 0 &&
			rf.log[i-firstIndex].Term == rf.log[args.PrevLogIndex-firstIndex].Term; i-- {
			conflictIndex = i
		}
	...
	// Append entries
	i, j := args.PrevLogIndex-firstIndex+1, 0
...
func (rf *Raft) sendAppendEntries(server int, args *AppendEntriesArgs, reply *AppendEntriesReply) bool {
	...
	if reply.Success {
		...
		for ; newNextIndex >= 0; newNextIndex-- {
			if rf.log[newNextIndex-firstIndex].Term == reply.ConflictTerm {
				break
			}
		}
...
func (rf *Raft) InstallSnapshot(args *InstallSnapshotRequest, reply *InstallSnapshotReply) {
	...
	go func() {
		rf.applyCh <- ApplyMsg{SnapshotValid: true,
			Snapshot:      args.Data,
...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	...
	rf.log = append(rf.log, LogEntry{Term: term, Index: rf.getLastIndex() + 1, Command: command})
	rf.persist()
...
func (rf *Raft) broadcastHeartbeat() {
	...
	for i := 0; i < len(rf.peers); i++ {
		if i != rf.me {
			firstIndex := rf.getFirstIndex()
			entries := rf.log[rf.nextIndex[i]-firstIndex:]
			prevTerm := rf.log[rf.nextIndex[i]-firstIndex-1].Term
			leaderCommit := rf.commitIndex
			if rf.nextIndex[i]-1 < firstIndex {
				go func(server int) {
